...
def capture_user_persona(responses):
	...
	# Mark persona as captured locally first
	frappe.db.set_single_value("LMS Settings", "persona_captured", True)
	
	# Try to send to external API, but don't fail if it doesn't work
	try:
		...
		# Log HTTP errors (like 417) but don't fail the request
		frappe.log_error(
			message=f"External persona capture API returned error: {str(e)}",
			title="Persona Capture API Error"
		)
	except Exception as e:
		# Log any other errors but don't fail the request
		frappe.log_error(
			message=f"Failed to send persona data to external API: {str(e)}",
			title="Persona Capture Error"
		)
	
	# Return success response even if external API fails
	return {"message": {"name": "Persona captured locally"}}
...
def get_profile_details(username):
	...
	if details:
		...
		if details.get("user_image"):
			details.user_image = get_url(details.user_image)
		if details.get("cover_image"):
			details.cover_image = get_url(details.cover_image)
