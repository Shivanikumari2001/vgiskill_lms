...
def get_chart_data(
	...
	# Frappe v15 requires SQL expressions for aggregate functions, not dictionaries
	sum_field = f"sum({value_field})" if value_field != "1" else "sum(1)"
	data = frappe.db.get_all(
		doctype,
		fields=[datefield, sum_field, "count(*)"],
		filters=filters,
		group_by=datefield,
		order_by=datefield,
		as_list=True,
