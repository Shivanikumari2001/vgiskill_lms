...
# Runtime directories
# -------------------------
RUN mkdir -p sites config/pids logs sites/assets
...
# Install node deps + build assets (NO bench commands)
RUN for app in frappe lms payments; do \
      if [ -f "apps/$app/package.json" ]; then \
        cd apps/$app && yarn install && yarn build || true; \
      fi; \
    done
